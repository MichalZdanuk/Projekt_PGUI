    ...
    <RequiredAuth>
      <div className="widgetCard widgetCardBackground">
        ...
        <div className="centeredElement">
          <RankingTypeButton type="MOSTTURNOVER" isChosen={sortedType === "MOSTTURNOVER" ? "yes" : "no"}
            handleTypeClick={handleTypeClick}/>
          <RankingTypeButton type="MOSTAMOUNT" isChosen={sortedType === "MOSTAMOUNT" ? "yes" : "no"}
            handleTypeClick={handleTypeClick}/>
          <RankingTypeButton type="LEASTVIEWS" isChosen={sortedType === "LEASTVIEWS" ? "yes" : "no"}
            handleTypeClick={handleTypeClick}/>
          <RankingTypeButton type="LEASTAMOUNT" isChosen={sortedType === "LEASTAMOUNT" ? "yes" : "no"}
            handleTypeClick={handleTypeClick}/>
        </div>
        <br />
        <div className="centeredElement leftAndRightMargins">
        <TableOfOffers offersList={receivedSellerOffers} chosenType={sortedType} />
        </div>
        <br />
      </div>
    </RequiredAuth>
...
function TableOfOffers(props){
  ...
  if(props.chosenType === "MOSTTURNOVER"){
    ...
    processedData = processedData.sort((a,b)=> b.numberOfItems - a.numberOfItems);
  } else if(props.chosenType === "LEASTVIEWS"){
    processedData = processedData.sort((a,b)=> a.numberOfViews - b.numberOfViews);
  } else if(props.chosenType === "LEASTAMOUNT"){
    processedData = processedData.sort((a,b)=> a.numberOfItems - b.numberOfItems);
  }
  processedData = processedData.slice(0,5);
  const listOfOffers = processedData.map((offer) => (<Offer
    name={offer.productName} key={offer.id}
     firstVal={(props.chosenType === "MOSTTURNOVER" || props.chosenType === "MOSTAMOUNT") ? offer.turnover : offer.numberOfViews}
     secondVal={offer.numberOfItems} />))
  
  return (<table className="center" border="2px">
    <col width="250px" />
    <col width="250px" />
    <col width="250px" />
    <col width="250px" />
  <tr className="adjustedTextLargeWidgetThumbnail">
    <th><TextAny text="image" /></th>
    <th><TextAny text="name" /></th>
    <th><TextAny text={(props.chosenType === "MOSTTURNOVER" || props.chosenType === "MOSTAMOUNT") ? "turnover" : "LEASTVIEWS"} /></th>
    <th><TextAny text="LEASTAMOUNT" /></th>
  </tr>
    {listOfOffers}
  </table>)
}
...
function Offer(props){
  ...
  <tr className="adjustedTextWidgetThumbnail">
    <td><img src={choosePicture(props.name)} alt="pic" height="50"/></td>
    <td>{props.name}</td>
